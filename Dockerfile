...
EXPOSE 8080
CMD ["/bin/sh", "-c", "sed -i \"s/listen 8080;/listen ${PORT:-8080};/g\" /etc/nginx/nginx.conf && su ziguser -c \"DATABASE_URL=$DATABASE_URL /usr/local/bin/logic\" & exec nginx -g 'daemon off;'"]