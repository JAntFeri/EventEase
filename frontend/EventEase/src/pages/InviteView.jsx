...
export default function InviteView({ eventData }) {
  ...
  const [votedDates, setVotedDates] = useState([]);
  ...
  return (
    <div className="py-12 md:py-16 px-4 max-w-xl mx-auto">
      <span className="inline-block text-[10px] font-bold uppercase tracking-wider bg-emerald-100 text-emerald-800 py-1 px-2.5 rounded-full mb-4">Vabilo od {organizerName}</span>
      <div className="mb-8">
      ...
      <form onSubmit={(e) => { e.preventDefault(); if(!guestName) return alert('Vpišite ime.'); if(!votedDates.length) return alert('Izberite termin.'); setSubmitted(true); }} className="space-y-8">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <CalendarPicker selectedDates={votedDates} onChange={setVotedDates} isPollMode={true} allowedDates={suggestedDates} />
        </div>
